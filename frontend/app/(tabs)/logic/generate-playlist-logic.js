...
export const useGeneratePlaylistLogic = () => {
  ...
  const [intervals, setIntervals] = useState([{ time: '', pace: '' }])
  ...
  const handleGenerate = () => {
    intervals.forEach(interval => {
      getRecommendations(interval.time, interval.pace)
    })
    addPlaylist()
  }
  ...
  const addPlaylist = () => {
    ...
    getTokens().then(token => {
      if (token[0]) {
        fetch("http://192.168.51.108:8000/cadence/api/playlist/", {
          method: "POST",
          headers: {
  ...
  const setIntervalTime = (index, time) => {
    const newIntervals = [...intervals];
    newIntervals[index].time = time;
    setIntervals(newIntervals);
  }

  const setIntervalPace = (index, pace) => {
    const newIntervals = [...intervals];
    newIntervals[index].pace = pace;
    setIntervals(newIntervals);
  }

  const addInterval = () => {
    setIntervals([...intervals, { time: '', pace: '' }]);
  }
